...
best_net = DQN(n_observations, n_actions).to(device)
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if update_interval_counter % update_interval == 0:
        ...
        if done:
            ...
            if t + 1 >= max_episode_duration:
                ...
                best_net.load_state_dict(target_net.state_dict())
                torch.save(best_net.state_dict(), './model/dqn_best_model.pth')
                print(f"{i_episode}-th episode: New best model saved with duration: {max_episode_duration}")
            if t + 1 >= 500:
                max_episode_length += 1
            else:
                max_episode_length = 0
            if max_episode_length == 20:
                end_flag = True
                break
...
while True:
    state = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
    action = best_net(state).max(1).indices.view(1, 1)
    observation, reward, terminated, truncated, _ = env.step(action.item())
    steps_done += 1
    done = terminated or truncated
    if done:
